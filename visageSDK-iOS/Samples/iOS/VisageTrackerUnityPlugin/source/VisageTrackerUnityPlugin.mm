...
extern "C" {
	...
	void _initTracker (char* config)
	{
        ...
        //initializeLicenseManager("xxx.vlc");
    ...
    int _getActionUnitCount()
	{
		...
		return observer->data.actionUnitCount;
	}
	...
	void _getActionUnitValues(float* values)
	{
		...
		int leftIndex = -1;
		int rightIndex = -1;
		for (int i = 0; i < observer->data.actionUnitCount; i++)
		{
			if (!strcmp(observer->data.actionUnitsNames[i], "au_leye_closed"))
			{
				leftIndex = i;
			}

			if (!strcmp(observer->data.actionUnitsNames[i], "au_reye_closed"))
			{
				rightIndex = i;
			}
		...
		if (leftIndex >= 0 && observer->data.actionUnitsUsed[leftIndex] == 0) {
			observer->data.actionUnits[leftIndex] = observer->data.eyeClosure[0];
		}
		if (rightIndex >= 0 && observer->data.actionUnitsUsed[rightIndex] == 0) {
			observer->data.actionUnits[rightIndex] = observer->data.eyeClosure[1];
		}
        
		memcpy(values, observer->data.actionUnits, observer->data.actionUnitCount * sizeof(float));
	}
	...
	const char* _getActionUnitName(int index)
	{
		...
		return observer->data.actionUnitsNames[index];
	}
	...
	bool _getActionUnitUsed(int index)
	{
		...
		return observer->data.actionUnitsUsed[index] == 1;
	}
	...
	bool _getFeaturePoints2D(int number, int* groups, int* indices, float* positions)
	{
		...
		for (int i = 0; i < number; i++)
		{
			int group = groups[i];
			int index = indices[i];
			const float* position = observer->data.featurePoints2D->getFPPos(group, index);
